    ...
    if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, slot, PID),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) > NULL_ID)
      {
        ...
        pi[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,PX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
        pi[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,PY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
        pi[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,PZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
#ifdef VISCOELASTIC
        ...
        vi[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,VX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
        vi[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,VY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
        vi[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,slot,VZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
#endif
        delta_Fi[0] = 0.0;
        delta_Fi[1] = 0.0;
        delta_Fi[2] = 0.0;
        ...
        for (int inner_slot=slot+1; inner_slot<MAX_NUMBER_OF_PARTICLES_PER_CELL; inner_slot++)
          {
            ...
            if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, inner_slot, PID),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) == NULL_ID)
              continue;
            ...
            pj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,PX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
            pj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,PY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
            pj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,PZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
#ifdef VISCOELASTIC
            ...
            vj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
            vj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
            vj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
#endif
            delta_Fj[0] = 0.0;
            delta_Fj[1] = 0.0;
            delta_Fj[2] = 0.0;
                ...
                if (dij < 2*PARTICLE_RADIUS)
                  {
                    ...
                    calSetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
                       calGetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fj[0]);
                    calSetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
                       calGetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fj[1]);
                    calSetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
                       calGetBuffer3DElement(REAL_SUBSTATE(Q_next,inner_slot,FZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fj[2]);
            ...
            if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, inner_slot, PID),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) == BORDER_ID)
              {
                ...
                Nj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
                Nj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
                Nj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,inner_slot,VZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z);
#ifdef VISCOELASTIC
                vj[0] = 0.0;
                vj[1] = 0.0;
                vj[2] = 0.0;
        ...
        calSetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
           calGetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FX),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fi[0]);
        calSetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
           calGetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FY),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fi[1]);
        calSetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z,
           calGetBuffer3DElement(REAL_SUBSTATE(Q_next,slot,FZ),X_CELLS,Y_CELLS,cell_x,cell_y,cell_z) + delta_Fi[2]);
      }
}
        ...
        for (int n = 1; n<ca->sizeof_X; n++)
          {
            for (int outer_slot=0; outer_slot<MAX_NUMBER_OF_PARTICLES_PER_CELL; outer_slot++)
              {                
                ...
                if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, outer_slot, PID),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS)) == NULL_ID)
                  continue;
                ...
                pj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,PX),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                pj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,PY),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                pj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,PZ),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                ...
                vj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VX),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                vj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VY),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                vj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VZ),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
#endif
                //if (calGetX3Di(ca, Q.ID[outer_slot],cell_x,cell_y,cell_z,n) > NULL_ID)
                //if (calGetBuffer3DElement(Q_ID_current[outer_slot],X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS)) > NULL_ID)
                if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, outer_slot, PID),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS)) > NULL_ID)
                ...
                if (calGetBuffer3DElement(INT_SUBSTATE(ID_current, outer_slot, PID),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS)) == BORDER_ID)
                  {
                    ...
                    Nj[0] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VX),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                    Nj[1] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VY),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
                    Nj[2] = calGetBuffer3DElement(REAL_SUBSTATE(Q_current,outer_slot,VZ),X_CELLS,Y_CELLS,calGetToroidalX(cell_x+Xi[n],X_CELLS),calGetToroidalX(cell_y+Xj[n],Y_CELLS),calGetToroidalX(cell_z+Xk[n],Z_CELLS));
    #ifdef VISCOELASTIC
                    vj[0] = 0.0;
                    vj[1] = 0.0;
                    vj[2] = 0.0;
